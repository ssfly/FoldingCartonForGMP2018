...
\begin{figure}
	...
	\caption{\xjmd{(a) Compared with STUDIO, more participants prefer our system considering both the operation simplicity and modeling time. (b) Comparison of fabrication times with/without video. It took much longer time for common users to fabricate a carton from a 2D layout only without our guide process. (c) Our system is able to automatically refine the inaccurate 2D layout, while STUDIO only generates the folded shape based on user assigned angles.} }
	\label{fig:userstudy}
\end{figure}
