...
{\color{blue}{Based on the observation above, we decide to set $\pi/2$ as the value of rotation angle to each fold edge. First, the face adjacent graph of a layout is shown in Figure~\ref{fig:midresult} (a)} where each face is a node. and if two faces are connected through a fold edge. Then by regarding the face with maximum area as fixed face, our system breadth first searches its adjacent faces and rotate the adjacent face around the current node with $\pi/2$, and finally the initialization result has been generated as Figure~\ref{fig:midresult} (e).} 
