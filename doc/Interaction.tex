...
%
Any pair of vertexes $\mathbf{v}_a$, $\mathbf{v}_b$ that are in {\color{blue}{different}} faces are detected as mergeable, if $|\mathbf{v}_a-\mathbf{v}_b|<\epsilon$, and there is an edge connecting to $\mathbf{v}_a$ having the equal length with one edge connecting to $\mathbf{v}_b$,
%
\cxj{$\epsilon$ = ? in our experiments.}{\color{blue}{where $\epsilon$ is set as 50mm in our experiments.}}
...
\begin{figure}
	\centering
	 \subfigure[Vertex Merging]{
	 	...
	 	\includegraphics[width=0.45\textwidth]{images/merging.pdf}}
	 \hspace{0.1in}
	 \subfigure[Symmetry Detection]{
	 	\label{fig:subfig:b} %% label for second subfigure
	 	\includegraphics[width=0.45\textwidth]{images/symmetry.pdf}}
