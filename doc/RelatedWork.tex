...
\reply{Although with the same goal that constructing 3D models from 2D layouts including vertexes and edges as line drawing problem, theres is a one-to-one correspondence in the vertexes between 2D layouts and 3D model in line drawings while ours is many-to-one correspondence and the construction process is actually different.} 
